...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        // Insert order
        $stmt = $conn->prepare("INSERT INTO orders (order_cost, order_status, customer_id, customer_phone, customer_city, customer_address, order_date) 
                                VALUES (?, 'pending', ?, ?, ?, ?, ?)");
        
        if ($stmt === false) {
            throw new Exception("Error in order prepare: " . $conn->error);
        }
        ...
        foreach ($cart_items as $item) {
            ...
            if ($stmt === false) {
                throw new Exception("Error in order_items prepare: " . $conn->error);
            }
            ...
            if ($stmt->error) {
                throw new Exception("Error executing order_items insert: " . $stmt->error);
            }
        }
        ...
        $stmt = $conn->prepare("INSERT INTO payments (order_id, customer_id, payment_method, transaction_id, payment_date)
                                VALUES (?, ?, ?, ?, ?)");
        if ($stmt === false) {
            throw new Exception("Error in payments prepare: " . $conn->error);
        }
        $stmt->bind_param('iisss', $order_id, $customer_id, $payment_method, $transaction_id, $payment_date);
        $stmt->execute();
        if ($stmt->error) {
            throw new Exception("Error executing payments insert: " . $stmt->error);
        }
                                    ...
                                    <li class="account">
                                        ...
                                        <ul class="account_selection">
                                            <?php if (isset($_SESSION['customer_id'])): ?>
                                                <li><a href="../gems-customer-pages/logout-customer.php"><i class="fa fa-sign-out" aria-hidden="true"></i>Logout</a></li>
                                            <?php else: ?>
